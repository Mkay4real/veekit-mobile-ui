const getInitials = (input: string = '', num: number = 1): string => {
    ...
    if (words.length === 1 && num === 1) {
        result = String(words[0]?.[0]).toUpperCase()
    }

    if (words.length === 1 && num === 2) {
        result =  String(words[0]?.[0]).toUpperCase() +  String(words[0]?.[1]).toUpperCase()
    }

    // For two words, take the first letter of each word
    if (words.length === 2) {
        result =  String(words[0]?.[0]).toUpperCase() +  String(words[1]?.[0]).toUpperCase()
    }

...
const createColorScheme = (
  ...
  for (let i = 1; i <= 12; i++) {
    colorScheme[`${prefix}${i}`] = colorSource[`${sourcePrefix}${i}`] as string;
  }
  return colorScheme;
};
...
const createTextColorScheme = ({
  ...
  createColorStateText({
    ...
    inverse: color[`${string}1`] as string,
...
const createBorderColorScheme = ({
  ...
  createColorStateBorder({
    primary: (isGray ? alpha[`${string}A6`] : alpha[`${string}A8`]) as string,
    light: alpha[`${string}A7`],
    lighter: alpha[`${string}A6`],
    strong: alpha[`${string}A11`] as string,

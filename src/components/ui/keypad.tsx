  ...
  return (
    <View style={tw`p-xs gap-[5] ${containerClassName}`}>
      {keys.map((row, rowIndex) => (
        <View key={rowIndex} style={tw`flex-row w-full  justify-between mb-btn-hg-1`}>
          {row.map((key, keyIndex) => (
            <TouchableOpacity
              ...
              >
              {key === KEY_BIO ? (
                <FingerprintIcon color={colors.dark.type.accent.DEFAULT} size={20} />
              ) : key === KEY_BACKSPACE ? (
                <DeleteIcon color={colors.dark.type.accent.DEFAULT} size={20} />
              ) : (
                <Text style={tw`text-xl-bold text-gray-800 dark:text-dark-type-gray`}>
                  {key}
                </Text>
              )}
            </TouchableOpacity>

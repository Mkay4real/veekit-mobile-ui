...
const ListItem: React.FC<ListItemProps> = ({
  ...
  return (
    <Pressable
      onPress={onPress}
      style={tw`flex-row items-center justify-between py-2 px-3   ${separator ? 'border-b border-gray-300 ' : 'border-b-0'}`}
    >
      ...
      <View style={tw`flex-1 ${order === 'reverse' ? 'flex-col-reverse' : ''}`}>
        ...
        {!!subtitle && (
          <Text style={tw`text-xs-body text-gray-500`}>{subtitle}</Text>
        )}
        {!!extra && (
          <Text style={tw`text-md-title font-bold text-gray-800`}>{extra}</Text>
        )}
      </View>
      {!!trailingIcon && (
        <View className={`ml-lg justify-center self-center items-center`}>
          {trailingIcon}
        </View>
      )}
      {subTrigger && (
